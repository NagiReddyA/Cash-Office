...
export class PrintbankslipComponent {
  ...
  addDenomination(printDepositSlip) {
    ...
    if (TotalAmount <= this.ttlCash && updateAmt <= this.ttlCash) {
      var denominationcode = this.printDepositSlip.value.denominationCode;
      var amount = this.printDepositSlip.value.Amount;
    ...
    this.http.post(apiURL + '/getReceipts/',
      ...
      if(this.receiptsData == null){  
       alert("Already submitted for this selected date");
        ...
        this.ChequeData = this.receiptsData.filter(app => app.payMethodDesc == "CHEQUE1");
        ...
        this.CashData = this.receiptsData.filter(app => app.payMethodDesc == "CASH1");
